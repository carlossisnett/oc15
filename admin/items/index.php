...
<?php endif;?>
<?php
$user_type = isset($_SESSION['userdata']['type']) ? (int)$_SESSION['userdata']['type'] : 0;
?>
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title">Historial por Artículo</h3>
	...
	<div class="card-body">
		<div class="row mb-3">
			<div class="col-md-6">
				<label for="search_inventory_item">Buscar artículo de inventario</label>
				<input type="text" id="search_inventory_item" class="form-control form-control-sm rounded-0" placeholder="Escribe código SAP o descripción (mínimo 3 caracteres)">
				<small class="text-muted">Al seleccionar un artículo, se abrirá su historial.</small>
			</div>
		</div>
        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped">
				...
				<tbody>
					<?php 
					$i = 1;
					$qry = $conn->query("SELECT * from `item_list` order by (`name`) asc LIMIT 100");
					...
					?>
						<tr>
							...
							<td>
								<a href="?page=items/history&item_id=<?php echo $row['id'] ?>" title="Ver historial del artículo">
									<?php echo $row['name'] ?>
								</a>
							</td>
							...
							<td align="center">
								 <button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon py-0" data-toggle="dropdown">
				                  ...
				                  <div class="dropdown-menu" role="menu">
									...
									<a class="dropdown-item" href="?page=items/history&item_id=<?php echo $row['id'] ?>"><span class="fa fa-history text-info"></span> Historial</a>
				                    <div class="dropdown-divider"></div>
...
<script>
	$(document).ready(function(){
		$('#search_inventory_item').autocomplete({
			...
			select:function(event, ui){
				if(ui && ui.item && ui.item.id){
					location.href = "?page=items/history&item_id="+ui.item.id;
				}
			},
			change:function(event, ui){
				if(!ui.item && $('#search_inventory_item').val().trim() !== ''){
					alert("Por favor, selecciona un artículo válido de la lista.");
					$('#search_inventory_item').val('');
				}
			}
		});
