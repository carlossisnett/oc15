			...
			<table class="table table-hover table-striped">
				...
				<tbody>
					...
					//$qry = $conn->query("SELECT po.*, CONCAT_WS(' ', u.firstname, u.lastname) as sname FROM `po_list` po inner join `users` u on po.username = u.username order by unix_timestamp(po.date_updated) ");
						...
						$strqry = "SELECT si.*, CONCAT_WS(' ', u.firstname, u.lastname) as sname FROM `solicitud_de_inventario` si inner join `users` u on si.username = u.username where si.username = '" . $_SESSION['userdata']['username'] . "' order by unix_timestamp(si.date_updated)";
